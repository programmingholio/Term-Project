...
public class boss_controller : MonoBehaviour {
    ...
    void ForceAttack(double frame){
        ...
        p_rb2d.isKinematic = false;

        if( player_pos.x < current_position.x){
            //Apply substantial amount of force up, and to the left.
            p_rb2d.AddForce(new Vector2(-.5f * forceattack_speed, 10f * forceattack_speed), ForceMode2D.Force); 
        }    
    ...
    void FlyingAttack(double frame){
       ...
       //Teleport to either bottom right or bottom left. 
            switch (direction){
                case 1:
                    boss_transform.position =  new Vector3 (51f,1.7f,0f); 
                    dir = -1;
                    break;
                case 2:
                    boss_transform.position =  new Vector3 (31f,1.7f,0f); 
                    dir = 1;
                    break;
                default:
                    dir = 0;
